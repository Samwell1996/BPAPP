...
export class DeviceClass implements IDevice, IDeviceInternal {
  ...
  get isTablet() {
    return (
      isPad() ||
      (this.screenAspectRatio < 1.6 &&
        Math.max(this.window.width, this.window.height) >= 900)
    );
  }
  ...
  get isIphoneX() {
    return (
      ...
      isPad() &&
      !Platform.isTV &&
      (this.window.height >= 812 || this.window.width >= 812)
    );
  }
  ...
  addDimensionsEventListener(callback: any) {
    this.dimentsionSubscription = Dimensions.addEventListener(
      'change',
      callback,
    );
  }
